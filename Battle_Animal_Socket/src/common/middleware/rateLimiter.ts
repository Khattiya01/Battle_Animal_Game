...
const rateLimiter = rateLimit({
  legacyHeaders: true,
  limit: env.COMMON_RATE_LIMIT_MAX_REQUESTS ? parseInt(env.COMMON_RATE_LIMIT_MAX_REQUESTS.toString()) : 20,
  // message: 'Too many requests, please try again later.',
  message: `You have exceeded the ${env.COMMON_RATE_LIMIT_MAX_REQUESTS} requests limit!, please try again later.`,
  standardHeaders: true,
  windowMs: 15 * 60 * (env.COMMON_RATE_LIMIT_WINDOW_MS ? parseInt(env.COMMON_RATE_LIMIT_WINDOW_MS.toString()) : 1),
  keyGenerator,
});
